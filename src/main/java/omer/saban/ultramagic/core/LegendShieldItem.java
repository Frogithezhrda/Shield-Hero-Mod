...
public class LegendShieldItem extends SwordItem {
	   public LegendShieldItem(Properties p_43089_) {
		super(ToolMaterialInit.LEGEND, 5, -1, p_43089_);
...
Level world;
	@Override
	   ...
	   public void inventoryTick(ItemStack p_41404_, Level p_41405_, Entity entity, int p_41407_, boolean p_41408_) {
	       ...
	       if(player.getMainHandItem() != null && player.getMainHandItem().getItem() instanceof LegendShieldItem) {
	   		  player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 1, 02,false,false));
	   ...
	   public InteractionResultHolder<ItemStack> use(Level world, Player playerIn, InteractionHand p_43101_) {
		      ...
		      if(ClientForgeEvents.value == 4) {
		    	  ...
		    	  for(int y = 0; y < 15002; ++y) {
			    	  world.addParticle(ParticleTypes.FLAME, playerIn.getX() + random.nextDouble() * 40.0D + random.nextDouble() * -40.0D, playerIn.getY() + random.nextDouble() * 0.5D,
								playerIn.getZ() + random.nextDouble() * 40.0D + random.nextDouble() * -40.0D, 0.0D, 0.0D,0.0D);
			    	  }
